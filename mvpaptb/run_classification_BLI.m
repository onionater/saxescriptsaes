function run_classification_BLI(task, subjectlist, runsincluded, varargin)
...
rootdir=['/mindhive/saxelab2/'];
folderprefix='';
masksubset={}; % {'whole_brain_skullstripped'}; %{'VMPFC_tomloc_ind'} %based on names in subjinfo mvpa_mask_description
discsubset={}; %{'faceVScontext', 'negVSpos'}; %, 'negVSposONE', 'negVSposTWO'}; %based on names in discrimination .mat file
studydir=[rootdir study '/'];
printandsave_script='/mindhive/saxelab/scripts/aesscripts/mvpaptb/printandsave_mvpaptb';
uselotsofspace=1; %set this to 1 if you want to be a space gobbler (and get some sanity check figs). save this for when debugging a new analysis/feature
checkbeforedelete=1;
...
perm_its=0; %if doing permutation analysis, will generate this many iterations (default==1)
...
other_args.hemodynamic_delay=4;  %this could be 4 or 6!
...
other_args.fixed=1; %this specifies whether you are using rank (1) or threshold (0) based feature selection
other_args.fsthreshold=0.05; % this specifies (initial) p-value for thresholding stat maps from anova for each xval fold
other_args.fixednum=80; %this specifies the number of voxels to take if using fixed rank-based feature selection  (for people with less, take all the voxels)
other_args.fsfunc='activity'; %default='activity' (F for contrast of activate part of task (used in discrim) vs rest), 'anova' (F on betas of conditions in discrimination),  'allcondandrestF'(don't use allcondandrestF... it's fucked up and activity makes more sense)
other_args.voxelthreshold=80; % freak out if xval fold has fewer than this many voxels (only matters for threshold based FS)
other_args.hpfilter=1; %default=0, %128  -- in PTB
other_args.detrend=1; %default=0,  -- detrends across entire run 
other_args.globalnorm=0; %%default=0,  (subtract from each timepoint global mean across voxels. NOTE as currently implemented global mean is based on gray matter voxels only (since pattern is masked with ws*img), which will mean global mean is even more biased by real functional activations. USE WITH CARE.)
...
other_args.notes='maybe finalized?'; % write yourself some handy notes about this analysis

%parameters that require more thought)
other_args.classifier='libsvm' % 'lda' 'ridge', 'gnb'  %%CHANGE THIS TO DESIRED
...
maxmask=numMasks;
onemaskdone=0; % changes to one when first mask has been completed. subjstructure is saved for that mask.
...
xvalselector='runs'; %%assumes that all classifications can be done across runs. some will additionally be done with an across stimulus xval
[subjectsave, results, printregressor, fsoutputthreshold]=mvpaptb_classify(study, task, runsincluded, mvparootdir, subjectID, subj, binarized, convolved, condnames, mask_description, xvalselector, averagingselector, averagingselectorcomb, other_args, class_args);
