...
subject=subjectlist{s}
   ...
   % boldlist=boldlist(runsrange)    
    runsincluded='all runs' % [int2str(runsrange(1)) 'to' int2str(runsrange(end))];
...
IPS=numTimepoints/numruns;  %assumes matched across people; could get out of the behavioral instead 
...
for c=1:numConds
    ...
    for o=1:numonsets
        onset=condonsets(o);
        binarizedrunmatrix(onset:onset+(condduration-1),c)=1;  %this is where one should fx the duaration for different ds
    end
end
...
combinedselector(restregressor==1 & art_selectors==1)=1; 
    %binary vector of good time points 
...
% % crossrunstimselector=stimselector+(eoselectors-1).*2; %one and two are odd, three and four are even (1= persons, 2= contexts)
% % crossrunsONEselector=crossrunstimselector.*combinedselector;
...
% % crossrunsTWOselector=crossrunstimselector.*combinedselector;
...
% % combblockselectorreducedONE=combblockselector;
% % combblockselectorreducedTWO=combblockselector;
% % combblockselectorreducedONE(crossrunsONEselector==0)=0;
% % combblockselectorreducedTWO(crossrunsTWOselector==0)=0;
% % 
% % crossmatchedruns=rselectors;
...
%always start with all condition version!
alleight=reduce_regressors(binarizedregressors, convolvedregressors, [1 2 3 4 5 6 7 8], {'mu','fu','mh','fh','nu','su','nh','sh'}, 'none', blocklabel, combblockselector);  %binreg, conreg, conditions, labels, partition, events, goodtimepoints
...
negVSpos=reduce_regressors(binarizedregressors, convolvedregressors, [1 1 2 2 1 1 2 2], {'neg', 'pos'}, {'stimselector', 'crossmatchedruns'}, blocklabel, combblockselector);
negVSposONE=reduce_regressors(binarizedregressors, convolvedregressors, [1 1 2 2 1 1 2 2], {'neg', 'pos'}, 'crossrunsONEselector', blocklabel, combblockselectorreducedONE);
negVSposTWO=reduce_regressors(binarizedregressors, convolvedregressors, [1 1 2 2 1 1 2 2], {'neg', 'pos'}, 'crossrunsTWOselector', blocklabel, combblockselectorreducedTWO);
...
malenVSmalep=reduce_regressors(binarizedregressors, convolvedregressors, [1 0 2 0 0 0 0 0], {'malen', 'malep'}, 'none', blocklabel, combblockselector);
femalenVSfemalep=reduce_regressors(binarizedregressors, convolvedregressors, [0 1 0 2 0 0 0 0], {'femalen', 'femalep'}, 'none', blocklabel, combblockselector);
...
%save the relevant structures for the subject
 save([mvpadir 'subjinfo_BLI_main_' runsincluded], 'boldnames', 'maskname', 'mvpa_mask_description', 'allconds_binarized', 'allconds_convolved',  'allcond_names','blocklabels', 'blocklabelscomb', 'runselectors', 'evenoddselectors', 'artselectors', 'restselectors', 'combselectors', 'stimselector', 'crossmatchedruns', 'contextselector', 'genderselector', 'crossrunsONEselector', 'crossrunsTWOselector','foldlabels', 'TR', 'subjvoxelcount', 'voxelcountlabel');
 save([mvpadir 'discriminations_BLI_main_' runsincluded], 'alleight', 'faceVScontext', 'negVSpos', 'negVSposONE', 'negVSposTWO', 'negfVSposf', 'negcVSposc', 'maleVSfemale', 'socialVSnonsoc','socialnVSsocialp', 'nonsocnVSnonsocp', 'malenVSmalep', 'femalenVSfemalep');
    end
...
for cond=1:reducedNum
   ...
   reg(:,cond)=summedregressors;  %new cond regressor
