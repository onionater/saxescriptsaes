function analyMVPA_amy(roin, experiments,partition_names,condition_names_all,savedirectory,roinum, groups, group_loc,parametric,conORt)

% April 14 version -- Individual ROIs, parametric, 
% with contrast image creation  (Jorie)

%July 6 - generalized to add group ROISs  (Jorie)

%Edited by Jorie Feb 2011 for saxelab 

%Edited by AES Feb 2013 (small output organization changes. i.e. added call to mvpaData2csv so that data get saved as csv too)
%Edited by AES March 2013 to allow for analysis of betas or t-values
...
% options
center=1;           % (AES: as far as I can tell this variable is not used) between-conditions centering (patterns are normalized to a mean of zero in each voxel across all conditions)
...
%%conORt determines whether con or spmT images are used
                datafilename{nsubject,n1,n2}=fullfile(fileparts(current_spm),[[conORt],num2str(ic(n1,n2),'%04d'),'.img']); % subjects x partitions x conditions
            end
        end
...
save(['MVPA_data_' roin '.mat'],'pairwise','Rdata','Zdata','Zrawdata','Rrawdata','Data_part1','Data_part2','nvoxels')
mvpaData2csv(['MVPA_data_' roin '.mat'],['MVPA_data_' roin '.csv']);
